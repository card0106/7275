...
<!----产品数据列表----->
    <div class="tit f20"><a href="<?php echo U('charts','goods_id='.I('get.goods_id'));?>" class="f12">查看收益走势</a><a class="f12">查看明星渠道</a><a href="<?php echo U('goodsData','goods_id='.I('get.goods_id'));?>" class="f12">查看详细报表</a></div>
    <div class="con">
        <!---统计收入栏---->
    	...
    	<table border="0" cellpadding="0" cellspacing="0" class="productTable">
            ...
            <tr>
                ...
                <th width="3%">激活率</th>
                <th width="5%">每激活收益</th>
                <th width="10%">总平台收入</th>
                <th width="10%">总渠道支出</th>
                <th width="10%">总利润</th>
                <th width="3%">利润率</th>
                <th width="3%">占有率</th>
            </tr>
            ...
            <tbody id="filter">
                <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo ($row["real_num"]); ?>&nbsp;(<?php echo round($row['real_num']/$total['real_num']*100,2);?>%)</td>
                        <td><?php echo ($row["promote_num"]); ?>&nbsp;(<?php echo round($row['promote_num']/$total['promote_num']*100,2);?>%)</td>
                        <td><?php echo round($row['promote_num']/$row['real_num']*100,2);?>%</td>
                        <td>￥<?php echo round($row['umoney']/$row['promote_num'],2);?></td>
                        <td>￥<?php echo ($row["umoney"]); ?>&nbsp;(<?php echo round($row['umoney']/$total['umoney']*100,2);?>%)</td>
                        <td>￥<?php echo ($row["money"]); ?>&nbsp;(<?php echo round($row['money']/$total['money']*100,2);?>%)</td>
                        <td>￥<?php echo ($row["union_price"]); ?></td>
                        <td><?php echo 100-round($row['money']/$row['umoney']*100,2);?>%</td>
                        <td><?php echo round($row['union_price']/$total['price']*100,2);?>%</td>
